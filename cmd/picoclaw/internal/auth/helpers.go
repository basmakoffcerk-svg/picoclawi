...
const (
	supportedProvidersMsg = "supported providers: openai, openai-codex, qwen-cli, anthropic, google-antigravity"
	defaultAnthropicModel = "claude-sonnet-4.6"
	defaultQwenCliModel   = "qwen-oauth-cli"
	defaultQwenCliRef     = "qwen-cli/qwen3-coder-plus"
)
...
func authLoginCmd(provider string, useDeviceCode bool, useOauth bool) error {
	...
	case "openai-codex":
		return authLoginOpenAICodex(useDeviceCode)
	case "qwen-cli", "qwen":
		return authLoginQwenCLI()
	case "anthropic":
...
func authLoginQwenCLI() error {
	if _, err := exec.LookPath("qwen"); err != nil {
		return fmt.Errorf("qwen CLI not found in PATH; install Qwen CLI first")
	}

	// Trigger Qwen OAuth/device flow if needed.
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Minute)
	defer cancel()
	cmd := exec.CommandContext(ctx, "qwen", "-p", "Reply with: OK")
	out, err := cmd.CombinedOutput()
	if err != nil {
		output := strings.TrimSpace(string(out))
		if output == "" {
			return fmt.Errorf("qwen login/bootstrap failed: %w", err)
		}
		return fmt.Errorf("qwen login/bootstrap failed: %s", output)
	}
	...
	upsertModelConfig(appCfg, defaultQwenCliModel, defaultQwenCliRef, "oauth")
	appCfg.Agents.Defaults.ModelName = defaultQwenCliModel

	if err = config.SaveConfig(internal.GetConfigPath(), appCfg); err != nil {
		return fmt.Errorf("could not update config: %w", err)
	}

	fmt.Println("Qwen OAuth login successful!")
...
func authLoginOpenAI(useDeviceCode bool) error {
	return authLoginOpenAIProvider(useDeviceCode, "openai", "openai/gpt-5.4", "gpt-5.4")
}

func authLoginOpenAICodex(useDeviceCode bool) error {
	return authLoginOpenAIProvider(useDeviceCode, "openai-codex", "openai-codex/gpt-5.4", "gpt-5.4-codex")
}

func authLoginOpenAIProvider(useDeviceCode bool, providerKey, modelRef, modelName string) error {
	...
	appCfg, err := internal.LoadConfig()
	if err == nil {
		// Update or add provider model in ModelList
		foundProviderModel := false
		for i := range appCfg.ModelList {
			if appCfg.ModelList[i].Model == modelRef {
				appCfg.ModelList[i].AuthMethod = "oauth"
				appCfg.ModelList[i].Enabled = true
				foundProviderModel = true
				break
			}
		}

		// If no provider model in ModelList, add it.
		...
		// Update default model to use selected provider.
		appCfg.Agents.Defaults.ModelName = modelName
	...
	if provider != "" {
		...
		if err == nil {
			...
			for i := range appCfg.ModelList {
				...
				case "openai-codex":
					if isOpenAICodexModel(appCfg.ModelList[i].Model) {
						appCfg.ModelList[i].AuthMethod = ""
					}
				case "anthropic":
					if isAnthropicModel(appCfg.ModelList[i].Model) {
...
func upsertModelConfig(appCfg *config.Config, modelName, modelRef, authMethod string) {
	if appCfg == nil {
		return
	}
	for i := range appCfg.ModelList {
		if appCfg.ModelList[i].ModelName == modelName || appCfg.ModelList[i].Model == modelRef {
			appCfg.ModelList[i].ModelName = modelName
			appCfg.ModelList[i].Model = modelRef
...
// isOpenAICodexModel checks if a model string belongs to openai-codex provider
func isOpenAICodexModel(model string) bool {
	return model == "openai-codex" ||
		strings.HasPrefix(model, "openai-codex/")
}
