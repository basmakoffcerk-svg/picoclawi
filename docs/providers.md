...
| `qwen-cli`   | LLM (Qwen OAuth via CLI)                | OAuth login via `qwen` browser flow                                   |
...
| **Qwen OAuth CLI** | `qwen-cli/`      | `qwen` local CLI process                           | CLI       | Run `qwen` once and complete OAuth login in browser              |
