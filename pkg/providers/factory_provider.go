...
func createCodexAuthProvider() (LLMProvider, error) {
	cred, _, err := loadCodexCredential()
	if err != nil {
		return nil, fmt.Errorf("loading auth credentials: %w", err)
	}
	if cred == nil {
		return nil, fmt.Errorf("no credentials for openai-codex. Run: picoclaw auth login --provider openai-codex")
	}
...
func CreateProviderFromConfig(cfg *config.ModelConfig) (LLMProvider, string, error) {
	...
	case "openai-codex":
		// Dedicated OpenAI Codex backend with OAuth/token auth.
		if cfg.AuthMethod != "oauth" && cfg.AuthMethod != "token" {
			return nil, "", fmt.Errorf(
				"auth_method oauth or token is required for %q protocol (run: picoclaw auth login --provider openai-codex)",
				protocol,
			)
		}
		provider, err := createCodexAuthProvider()
