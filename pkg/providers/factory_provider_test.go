...
func TestExtractProtocol(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "openai-codex with prefix",
			model:        "openai-codex/gpt-5.4",
			wantProtocol: "openai-codex",
			wantModelID:  "gpt-5.4",
		},
		{
			name:         "qwen-cli with prefix",
			model:        "qwen-cli/qwen3-coder-plus",
			wantProtocol: "qwen-cli",
			wantModelID:  "qwen3-coder-plus",
		},
	}
...
func TestCreateProviderFromConfig_OpenAICodexRequiresAuthMethod(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("CreateProviderFromConfig() expected error, got provider=%T modelID=%q", provider, modelID)
	}
	if !strings.Contains(err.Error(), "auth_method oauth or token is required") {
		t.Fatalf("error = %q, want auth_method guidance", err.Error())
	}
}
