...
func (p *CodexProvider) Chat(
	...
	for stream.Next() {
		...
		case "response.refusal.done":
			if evt.Refusal != "" {
				streamedText.WriteString(evt.Refusal)
			}
		case "response.output_item.done":
			...
			case "message":
				for _, c := range item.Content {
					...
					case "refusal":
						streamedText.WriteString(c.Refusal)
					}
				}
			case "function_call":
				args := map[string]any{}
				...
				streamedToolCalls = append(streamedToolCalls, ToolCall{
					ID:        item.CallID,
					Name:      item.Name,
					Arguments: args,
				})
	...
	if parsed.Content == "" && len(parsed.ToolCalls) == 0 {
		if streamedText.Len() > 0 || len(streamedToolCalls) > 0 {
...
func resolveCodexModel(model string) (string, string) {
	...
	if after, ok := strings.CutPrefix(m, codexProviderID+"/"); ok {
		m = after
	} else if after, ok := strings.CutPrefix(m, legacyOpenAIProviderID+"/"); ok {
		m = after
...
func createCodexTokenSource() func() (string, string, error) {
	return func() (string, string, error) {
		cred, providerID, err := loadCodexCredential()
		if err != nil {
			return "", "", fmt.Errorf("loading auth credentials: %w", err)
		}
		if cred == nil {
			return "", "", fmt.Errorf(
				"no credentials for %s. Run: picoclaw auth login --provider %s",
				codexProviderID,
				codexProviderID,
			)
		}
		...
		if cred.AuthMethod == "oauth" && cred.NeedsRefresh() && cred.RefreshToken != "" {
			...
			if err := auth.SetCredential(providerID, refreshed); err != nil {
...
func loadCodexCredential() (*auth.AuthCredential, string, error) {
	cred, err := getCredential(codexProviderID)
	if err != nil {
		return nil, "", err
	}
	if cred != nil {
		return cred, codexProviderID, nil
	}

	// Backward compatibility: old builds stored Codex OAuth under "openai".
	legacyCred, err := getCredential(legacyOpenAIProviderID)
	if err != nil {
		return nil, "", err
	}
	if legacyCred != nil {
		return legacyCred, legacyOpenAIProviderID, nil
	}
