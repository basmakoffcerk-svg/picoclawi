...
| [Qwen Code](https://github.com/QwenLM/qwen-code) | `qwen-cli/` | OAuth (free tier) | Browser login on first run via `qwen` CLI |
...
**Qwen OAuth (free tier) quick setup**
1. Install Qwen Code CLI and run `qwen` once.
2. Complete browser login with your `qwen.ai` account.
3. Add a model like `qwen-cli/qwen3-coder-plus` to `model_list`.

Qwen CLI stores OAuth credentials locally, so repeated login is usually unnecessary.
